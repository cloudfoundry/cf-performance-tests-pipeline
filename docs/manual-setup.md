...
1. Install the [aws CLI](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html) and authenticate with the ARI WG root account. Note that the user's permission must exceed those defined in `bootstrap.yml` for the `PipelineUser`.
...
1. Retrieve the access key and secret key from AWS Secrets Manager, either in your browser or with the `aws` CLI:
    ...
    aws secretsmanager get-secret-value \
      --region us-east-1 \
      --secret-id arn:aws:secretsmanager:us-east-1:<id>:secret:/cf-perf-tests-pipeline-<xyz> \
      --query SecretString \
      --output text
    ```
1. Authenticate with [Concourse's CredHub](../README.md#general-information) and store these as follows:
    ```bash
    credhub set --name /concourse/capi-team/aws-pipeline-user-id --type value --value <AWS_ACCESS_KEY_ID>
    credhub set --name /concourse/capi-team/aws-pipeline-user-secret --type value --value <AWS_SECRET_ACCESS_KEY>
    ```
...
Whatever the case, manually set the pipeline with the following command:
